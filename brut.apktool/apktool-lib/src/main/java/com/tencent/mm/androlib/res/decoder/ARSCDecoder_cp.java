...
public class ARSCDecoder_cp {

  private final static boolean DEBUG = false;

  private final static short ENTRY_FLAG_COMPLEX = 0x0001;
  private static final Logger LOGGER = Logger.getLogger(ARSCDecoder_cp.class.getName());
  ...
  private ARSCDecoder_cp(InputStream arscStream, ApkDecoder decoder) throws AndrolibException, IOException {
  ...
  private ARSCDecoder_cp(InputStream arscStream, ApkDecoder decoder, ResPackage[] pkgs) throws FileNotFoundException {
    mOldFileName = new LinkedHashMap<>();
    mCurSpecNameToPos = new LinkedHashMap<>();
    mShouldResguardTypeSet = new HashSet<>();
    mApkDecoder = decoder;
  ...
  public static ResPackage[] decode(InputStream arscStream, ApkDecoder apkDecoder) throws AndrolibException {
    try {
      ARSCDecoder_cp decoder = new ARSCDecoder_cp(arscStream, apkDecoder);
  ...
  public static void write(InputStream arscStream, ApkDecoder decoder, ResPackage[] pkgs) throws AndrolibException {
    try {
      ARSCDecoder_cp writer = new ARSCDecoder_cp(arscStream, decoder, pkgs);
      writer.writeTable();
    } catch (IOException ex) {
      throw new AndrolibException("Could not decode arsc file", ex);
    }
  ...
  private static class MergeDuplicatedResInfo {
    ...
    static class Builder {
      ...
      ARSCDecoder_cp.MergeDuplicatedResInfo create() {
        return new ARSCDecoder_cp.MergeDuplicatedResInfo(fileName, filePath, originalName, md5);
      }
    }
  }
